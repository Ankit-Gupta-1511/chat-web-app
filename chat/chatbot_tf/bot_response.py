...
This file contains the main code for the chatbot that is going to train on updated dataset.
"""
...
for key in dataset.keys():
    for inpt in dataset[key]['inputs']:
        current_intent = dataset[key]['intent']
        w = nltk.word_tokenize(inpt)
        # add to our words list
        words.extend(w)
        # add to documents in our corpus
        documents.append((w, current_intent))
        # add to our classes list
        if current_intent not in classes:
            classes.append(current_intent)
            ...
            for key in dataset.keys():
                ...
                if dataset[key]['intent'] == results[0][0]:
                    ...
                    if not 'context_filter' in dataset[key] or \
                        ...
                        # a random response from the intent
                        return print(np.random.choice(dataset[key]['response']))
